SELECT '-- 1. Login verification (name + phone) --';
SELECT c_custKey
FROM customer
WHERE c_name = 'Fiona'
...
SELECT '-- 4. the amount of items you added from your store inventory --';
UPDATE stock
...
SELECT '-- 5. Calculate and update the order total --';
UPDATE transactions
SET t_totalpayment = (
...
SELECT '-- 6. Payment/Order Confirmation (Status C = Completed) 근데 이미 C 야  --';
UPDATE transactions
...
SELECT '-- 8. Customers order history (5 most recent) --';
...
SELECT '-- 9. Show products only from stores in my city (personalized) --';
